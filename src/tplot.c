...
{
  uint64_t *event_count = (uint64_t *)user_data;

  (*event_count)++;

  /* Print first 10 events for verification */
  if (*event_count <= 10) {
    printf("Event %lu: %s:%u -> %s:%u proto=%s time=%ld.%06u\n",
           *event_count,
  ...
  /* TODO: Add to Hilbert curve data structures */
 ...
 *
 ****/
PRIVATE int processHoneypotFile(const char *fName)
{
  ...
  if (!processGzipFile(fName, honeypotEventCallback, &event_count)) {
...
int processFile(const char *fName)
{
  ...
  /* Check if this is a gzip compressed honeypot log file */
  fNameLen = strlen(fName);
  if (fNameLen > 3 && strcmp(fName + fNameLen - 3, ".gz") == 0) {
    /* Process as honeypot sensor log */
    return processHoneypotFile(fName);
  }
